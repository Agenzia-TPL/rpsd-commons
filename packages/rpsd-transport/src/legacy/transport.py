...
def route_request(event):
    ...
    where_header = headers.get("x-raps-ingest_where") or headers.get(
        "X-RAPS-INGEST_WHERE"
    )
    ...
    if is_attachment:
        if where_url:
            raise ValueError("Cannot provide both attachment and a where clause")
    ...
    if where_url:
        ...
        # This case should not be reached due to the check above, kept for safety
        raise ValueError(
            "Must provide an attachment, an X-RAPS-INGEST_WHERE header, "
            "or a request body"
        )
...
def handle_attachment(event, who=None, what=None):
    ...
    return save_to_s3(
        ...
        filename,
        content_type or "application/xml",
        who=who,
        what=what,
    )
    ...
    with urllib.request.urlopen(where_url) as response:
        if response.status == 200:
            ...
            return save_to_s3(
                ...
                filename,
                content_type,
                source_url=where_url,
                who=who,
                what=what,
            )
        else:
            raise Exception(f"Failed to download from URL: Status {response.status}")
