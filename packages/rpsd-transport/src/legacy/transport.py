...
def route_request(event):
    ...
    if provided_options.count(True) != 1:
        raise ValueError('Must provide exactly one of: a where clause or a request body')
    ...
    if where_url:
        ...
        raise ValueError('Must provide an attachment, an X-RAPS-INGEST_WHERE header, or a request body')
